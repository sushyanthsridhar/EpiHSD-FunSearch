"""
train_autoencoder_latent6_v2.py
────────────────────────────────────────────────────────────────
Full training run using the best config from
tune_hyperparams_latent6_v2.py.

INSTRUCTIONS: after tuning finishes, copy the printed best config
values into the CONFIG block below, then run:
  python3 train_autoencoder_latent6_v2.py

Outputs:
  best_model_latent6_v2.pt
  latent_representations_latent6_v2.csv
  training_loss_latent6_v2.png
  latent_pairs_latent6_v2.png
────────────────────────────────────────────────────────────────
"""
...
# ── CONFIG: fill in from best_config_latent6_v2.txt after tuning ─
LATENT_DIM      = 6
EPOCHS          = 1200
PATIENCE        = 180
WEEKS_PER_YEAR  = 52

# Best config from trial 155, v2 Optuna run, val_r2=0.8893
...
ACTIVATION      = "elu"
MSE_WEIGHT      = 0.4563
SCHEDULER       = "plateau"       # none, plateau, or cosine_wr
OPTIMIZER_NAME  = "adam"          # adam or adamw
CLIP_NORM       = 0.0             # 0.0 = off
# ─────────────────────────────────────────────────────────────────
...
print(f"Training autoencoder v2  LATENT_DIM={LATENT_DIM} ...")
...
# Sort hidden_dims if monotone was best
hidden_dims_used = sorted(HIDDEN_DIMS, reverse=True) if MONOTONE else HIDDEN_DIMS

model = DengueAutoencoder(INPUT_DIM, LATENT_DIM, hidden_dims_used, PRE_BOTTLENECK,
                          DROPOUT, ACTIVATION, USE_BATCHNORM, SHALLOW_DECODER)
...
print(f"  Architecture: {[INPUT_DIM] + hidden_dims_used + ([PRE_BOTTLENECK] if PRE_BOTTLENECK else []) + [LATENT_DIM]}")
print(f"  Scheduler: {SCHEDULER}  Optimizer: {OPTIMIZER_NAME}  Clip: {CLIP_NORM}")
...
for epoch in range(1, EPOCHS + 1):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), os.path.join(OUT_DIR, "best_model_latent6_v2.pt"))
        patience_count = 0
    else:
        patience_count += 1
    ...
    if epoch % 50 == 0 or epoch == 1:
        ...
        print(f"  Epoch {epoch:4d}  train={train_loss:.5f}  val={val_loss:.5f}"
              f"  val_r2={val_r2:.4f}  lr={current_lr:.6f}"
...
model.load_state_dict(torch.load(os.path.join(OUT_DIR, "best_model_latent6_v2.pt")))
model.eval()
with torch.no_grad():
    val_r2  = r2_score(X_val, model(X_val)).item()
    val_mse = nn.functional.mse_loss(model(X_val), X_val).item()
print(f"\nFinal  val_r2={val_r2:.4f}  val_mse={val_mse:.6f}")
print(f"Previous best v1: val_r2=0.8888")
print(f"This run    v2: val_r2={val_r2:.4f}")
...
latent_df.to_csv(os.path.join(OUT_DIR, "latent_representations_latent6_v2.csv"), index=False)
print(f"Saved latent_representations_latent6_v2.csv  ({len(latent_df)} rows)")
...
ax.set_title(f"Training Loss v2  latent_dim={LATENT_DIM}  val_r2={val_r2:.4f}")
ax.legend()
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "training_loss_latent6_v2.png"), dpi=150)
plt.close()

# ── Latent space pairs plot ─────────────────────────────────────
...
plt.suptitle(f"Pairwise Latent Dims v2  latent_dim={LATENT_DIM}  val_r2={val_r2:.4f}", fontsize=13)
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "latent_pairs_latent6_v2.png"), dpi=150)
plt.close()
print("Saved training_loss_latent6_v2.png  latent_pairs_latent6_v2.png")
print(f"\n── DONE  latent_dim=6 v2 ───────────────────────────────────")
