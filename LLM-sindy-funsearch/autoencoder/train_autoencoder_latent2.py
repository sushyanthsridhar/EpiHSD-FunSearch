"""
train_autoencoder_latent2.py
────────────────────────────────────────────────────────────────
Best config from tune_hyperparams_latent2.py  (val_r2 = 0.8001)
  hidden_dims    : [512, 128, 1024]
  pre_bottleneck : 4
  shallow_decoder: True
  use_batchnorm  : False
  lr             : 0.004145
  dropout        : 0.2135
  weight_decay   : 0.000430
  batch_size     : 32
  activation     : relu
  mse_weight     : 0.3359

Run:
  python3 train_autoencoder_latent2.py
────────────────────────────────────────────────────────────────
"""
...
for epoch in range(1, EPOCHS + 1):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), os.path.join(OUT_DIR, "best_model_latent2.pt"))
        patience_count = 0
    else:
        patience_count += 1
...
model.load_state_dict(torch.load(os.path.join(OUT_DIR, "best_model_latent2.pt")))
model.eval()
with torch.no_grad():
    val_r2  = r2_score(X_val, model(X_val)).item()
...
latent_df.to_csv(os.path.join(OUT_DIR, "latent_representations_latent2.csv"), index=False)
print(f"Saved latent_representations_latent2.csv  ({len(latent_df)} rows)")
...
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "training_loss_latent2.png"), dpi=150)
plt.close()

fig, axes = plt.subplots(1, 2, figsize=(14, 6))
years = sorted(latent_df.year.unique())
yc = {y: plt.cm.plasma(i / len(years)) for i, y in enumerate(years)}
for ax_idx, (color_by, cmap_key) in enumerate([("province", None), ("year", yc)]):
    ax = axes[ax_idx]
    if color_by == "province":
        provs = sorted(latent_df.province.unique())
        pc = {p: plt.cm.tab20.colors[i % 20] for i, p in enumerate(provs)}
        for p in provs:
            m = latent_df.province == p
            ax.scatter(latent_df.loc[m, "z1"], latent_df.loc[m, "z2"],
                       color=pc[p], s=35, alpha=0.8)
        ax.set_title("2D Latent Space  by Province")
    else:
        for yr in years:
            m = latent_df.year == yr
            ax.scatter(latent_df.loc[m, "z1"], latent_df.loc[m, "z2"],
                       color=yc[yr], s=35, alpha=0.8, label=str(yr))
        ax.legend(title="Year", fontsize=7)
        ax.set_title("2D Latent Space  by Year")
    ax.set_xlabel("z1")
    ax.set_ylabel("z2")
    ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "latent_2d_latent2.png"), dpi=150)
plt.close()
print("Saved training_loss_latent2.png  latent_2d_latent2.png")
print(f"\n── DONE  latent_dim=2 ───────────────────────────────────")
