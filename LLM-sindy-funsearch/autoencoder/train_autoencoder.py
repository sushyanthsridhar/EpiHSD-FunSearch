...
WHAT THIS SCRIPT DOES (in plain terms):
  ...
  7. Saves latent_representations.csv (288 rows x 3 columns)
...
latent_df.to_csv(os.path.join(OUT_DIR, "latent_representations.csv"), index=False)
print(f"Saved latent_representations.csv  ({len(latent_df)} rows x 3 latent dims)")
...
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "latent_3d_by_province.png"), dpi=150)
plt.close()
print("Saved latent_3d_by_province.png")
...
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "latent_3d_by_year.png"), dpi=150)
plt.close()
print("Saved latent_3d_by_year.png")

print("\n── DONE ─────────────────────────────────────────────────")
print("Files saved in autoencoder/:")
print("  best_model.pt                 trained autoencoder weights")
print("  latent_representations.csv    288 province-year points in 3D space")
print("  training_loss.png             loss curve")
print("  latent_3d_by_province.png     3D scatter colored by province")
print("  latent_3d_by_year.png         3D scatter colored by year")
print("\nNext step: run sindy on latent_representations.csv")
