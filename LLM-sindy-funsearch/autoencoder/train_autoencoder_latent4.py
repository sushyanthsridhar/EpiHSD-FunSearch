"""
train_autoencoder_latent4.py
────────────────────────────────────────────────────────────────
Best config from tune_hyperparams_latent4.py  (val_r2 = 0.8552)
  hidden_dims    : [256]
  pre_bottleneck : 8
...
Run:
  python3 train_autoencoder_latent4.py
────────────────────────────────────────────────────────────────
"""
...
for epoch in range(1, EPOCHS + 1):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), os.path.join(OUT_DIR, "best_model_latent4.pt"))
        patience_count = 0
    else:
        patience_count += 1
...
model.load_state_dict(torch.load(os.path.join(OUT_DIR, "best_model_latent4.pt")))
model.eval()
with torch.no_grad():
    val_r2  = r2_score(X_val, model(X_val)).item()
...
latent_df.to_csv(os.path.join(OUT_DIR, "latent_representations_latent4.csv"), index=False)
print(f"Saved latent_representations_latent4.csv  ({len(latent_df)} rows)")
...
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "training_loss_latent4.png"), dpi=150)
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "latent_pairs_latent4.png"), dpi=150)
plt.close()
print("Saved training_loss_latent4.png  latent_pairs_latent4.png")
print(f"\n── DONE  latent_dim=4 ───────────────────────────────────")
