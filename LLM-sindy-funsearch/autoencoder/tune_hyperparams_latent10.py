...
Run:
  python3 tune_hyperparams_latent10.py

Outputs:
  optuna_results_latent10.csv
  best_config_latent10.txt
────────────────────────────────────────────────────────────────
"""
...
out_csv = os.path.join(HERE, f"optuna_results_latent10.csv")
res_df.to_csv(out_csv, index=False)
print(f"\nAll results saved to optuna_results_latent10.csv")

# ── Best config ────────────────────────────────────────────────
best = study.best_trial
cfg_path = os.path.join(HERE, f"best_config_latent10.txt")
with open(cfg_path, "w") as f:
    f.write(f"BEST HYPERPARAMETER CONFIG  latent_dim=10\n")
    f.write("=" * 40 + "\n")
...
print(f"Best config saved to best_config_latent10.txt")
