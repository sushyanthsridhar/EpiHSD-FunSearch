"""
tune_hyperparams_latent5.py
────────────────────────────────────────────────────────────────
Bayesian hyperparameter optimisation for LATENT_DIM = 5.
Identical to tune_hyperparams.py except LATENT_DIM and output
filenames. Test data is never loaded.

Run:
  python3 tune_hyperparams_latent5.py

Outputs:
  optuna_results_latent5.csv
  best_config_latent5.txt
────────────────────────────────────────────────────────────────
"""
...
out_csv = os.path.join(HERE, f"optuna_results_latent5.csv")
res_df.to_csv(out_csv, index=False)
print(f"\nAll results saved to optuna_results_latent5.csv")

# ── Best config ────────────────────────────────────────────────
best = study.best_trial
cfg_path = os.path.join(HERE, f"best_config_latent5.txt")
with open(cfg_path, "w") as f:
    f.write(f"BEST HYPERPARAMETER CONFIG  latent_dim=5\n")
    f.write("=" * 40 + "\n")
...
print(f"Best config saved to best_config_latent5.txt")
