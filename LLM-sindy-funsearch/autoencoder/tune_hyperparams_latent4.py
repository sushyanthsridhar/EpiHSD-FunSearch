"""
tune_hyperparams_latent4.py
────────────────────────────────────────────────────────────────
Bayesian hyperparameter optimisation for LATENT_DIM = 4.
Identical to tune_hyperparams.py except LATENT_DIM and output
filenames. Test data is never loaded.

Run:
  python3 tune_hyperparams_latent4.py

Outputs:
  optuna_results_latent4.csv
  best_config_latent4.txt
────────────────────────────────────────────────────────────────
"""
...
out_csv = os.path.join(HERE, f"optuna_results_latent4.csv")
res_df.to_csv(out_csv, index=False)
print(f"\nAll results saved to optuna_results_latent4.csv")

# ── Best config ────────────────────────────────────────────────
best = study.best_trial
cfg_path = os.path.join(HERE, f"best_config_latent4.txt")
with open(cfg_path, "w") as f:
    f.write(f"BEST HYPERPARAMETER CONFIG  latent_dim=4\n")
    f.write("=" * 40 + "\n")
...
print(f"Best config saved to best_config_latent4.txt")
