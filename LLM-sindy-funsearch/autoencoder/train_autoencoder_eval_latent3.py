...
This script is identical to the main training script EXCEPT:
  ...
  - It encodes ALL 9 years (2015-2023) and saves latent_all_years_latent3.csv
...
for epoch in range(1, EPOCHS + 1):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(),
                   os.path.join(OUT_DIR, "best_model_eval_latent3.pt"))
        patience_count = 0
    else:
        patience_count += 1
...
model.load_state_dict(torch.load(os.path.join(OUT_DIR, "best_model_eval_latent3.pt")))
model.eval()
...
all_latent_df.to_csv(os.path.join(OUT_DIR, "latent_all_years_latent3.csv"), index=False)
print(f"Saved latent_all_years_latent3.csv  ({len(all_latent_df)} rows, all 9 years)")
...
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "training_loss_eval_latent3.png"), dpi=150)
plt.close()
print("Saved training_loss_eval_latent3.png")

print("\n── DONE ──────────────────────────────────────────────────")
print("Next step: python3 visualize_latent_animation.py  (reads latent_all_years_latent3.csv)")
