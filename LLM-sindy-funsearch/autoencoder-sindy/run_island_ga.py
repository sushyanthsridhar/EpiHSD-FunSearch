...
Concept:
  Each island trains its own SINDy equation on a different random
  subset of provinces. Islands discover different dominant terms
  depending on which province cluster they see. The top 2 islands
  (by validation R squared across ALL provinces) are crossed via
  GA to produce a blended equation that captures structure from
  both. The crossed equation is then sent to Stage 3 test eval.

Pipeline:
  1. Load dim 6 autoencoder and latent representations.
  2. Define N_ISLANDS province subsets.
  3. Each island fits SINDy on its own province subset.
  4. All islands evaluated on ALL provinces on val transitions.
  5. Top 2 selected by mean val incidence dZ R squared.
  6. GA crossover: union of active terms, refit on all training
     pairs. N_CROSS_VARIANTS random partial crossovers also tried.
     Best crossover selected on validation.
  7. Stage 3 test evaluation with the crossed equation.

Run after training the dim 6 autoencoder:
  python3 autoencoder-sindy/run_island_ga.py

Outputs in autoencoder-sindy/island_ga/:
  island_scores.csv
  crossed_coefficients.csv
  eval_metrics_island_ga.csv
  plot_island_scores.png
  plot_incidence_island_ga.png
  plot_r2_heatmap_island_ga.png
────────────────────────────────────────────────────────────────
"""
...
OUT_DIR = os.path.join(HERE, "island_ga")
...
# Autoencoder config. Update bottom block after v2 tuning finishes.
MODEL_VERSION   = "v1"    # set to "v2" once tuning and retraining are done
...
# Island GA config
N_ISLANDS            = 8      # number of independent SINDy islands
PROVINCE_FRAC        = 0.52   # fraction of provinces per island (~14 of 27)
ISLAND_SEED_BASE     = 100    # random seeds for island province selection
N_CROSS_VARIANTS     = 20     # random partial crossover variants to try
CROSS_TERM_PROB      = 0.5    # prob of including unique-to-one-parent terms
...
model_path_v2 = os.path.join(AE_DIR, f"best_model_latent{LATENT_DIM}_v2.pt")
model_path_v1 = os.path.join(AE_DIR, f"best_model_latent{LATENT_DIM}.pt")
if MODEL_VERSION == "v2" and os.path.exists(model_path_v2):
    model_path = model_path_v2
    print(f"  Using v2 model: {model_path_v2}")
else:
    model_path = model_path_v1
    print(f"  Using v1 model: {model_path_v1}")
...
# Store training range for OOD clipping in Stage 3
...
# Polynomial library on scaled z
...
def build_pairs(df_ext, allowed_start_years, province_subset=None):
    """Build SINDy training pairs. If province_subset given, restrict to those provinces."""
    Zrow_list, dZ_list = [], []
    grp_iter = df_ext.groupby("province")
    for province, grp in grp_iter:
...
# Full training and validation pairs (all provinces)
...
def eval_xi_on_val(Xi_full):
    """Evaluate Xi on full validation pairs. Returns per-dim R squared and mean."""
...
for isl in range(N_ISLANDS):
    ...
    # Build full Xi with zeros for pruned terms (shape N_TERMS x LATENT_DIM)
    Xi_full = Xi_isl   # stlsq already returns full shape

    # Evaluate on ALL provinces (val) to measure generalisation
    val_r2s, val_r2_mean = eval_xi_on_val(Xi_full)
    # Train R squared on island subset
    preds_tr = Theta_tr_isl @ Xi_isl
    tr_r2s   = [r2_np(dZ_tr_isl[:, j], preds_tr[:, j]) for j in range(LATENT_DIM)]

    n_active  = int((np.abs(Xi_full) > 1e-12).any(axis=1).sum())
    # Fitness: val mean minus complexity penalty minus train-val gap penalty
...
# 1. Union crossover: include all terms active in either parent
...
# 2. Random partial crossovers: for each candidate, each unique term
#    from A or B is included with probability CROSS_TERM_PROB.
#    Terms in both parents are always included.
...
for v in range(N_CROSS_VARIANTS):
    # Always include terms in both parents
    cross_mask = in_both.copy()
    # Include unique-to-A terms with probability CROSS_TERM_PROB
    for ti in np.where(only_a)[0]:
        if rng.random() < CROSS_TERM_PROB:
            cross_mask[ti] = True
    # Include unique-to-B terms with probability CROSS_TERM_PROB
...
# Save final crossed coefficients
...
# ═══════════════════════════════════════════════════════════════
# STAGE 3 TEST EVALUATION (test data unlocked here)
# ═══════════════════════════════════════════════════════════════
print("\n" + "=" * 70)
...
for province in provinces_all:
    ...
    for test_year in TEST_YEARS:
        ...
        # Clip OOD external features to training range
        for fi, feat in enumerate(EXTERNAL_POOL_VALID):
...
prov_summary = eval_df.groupby("province")[["r2_incidence", "r2_didt"]].mean().sort_values(
...
metrics_save = eval_df.drop(columns=["pred_incidence","actual_incidence",
                                      ...
                                      "z_pred","z_actual"])
metrics_save.to_csv(os.path.join(OUT_DIR, "eval_metrics_island_ga.csv"), index=False)
print(f"\n  Saved eval_metrics_island_ga.csv")
...
# Plot 1: Island fitness comparison
...
axes[0].set_ylabel("Val R2 mean (all provinces)")
axes[0].set_title("Island Val R2 (red = selected for crossover)")
...
axes[1].set_title("Island Fitness Score")
axes[1].set_xticks(island_nums)
axes[1].grid(axis="y", alpha=0.3)
plt.suptitle(f"Island SINDy GA  (dim={LATENT_DIM}  N_islands={N_ISLANDS})", fontsize=12)
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "plot_island_scores.png"), dpi=150)
plt.close()

# Plot 2: Incidence curves for 8 sample provinces
...
plt.suptitle(
    f"Island GA  Predicted vs Actual  (Test 2022-2023)\n"
    f"Incidence R2  mean={eval_df['r2_incidence'].mean():.4f}  "
    f"median={eval_df['r2_incidence'].median():.4f}",
    fontsize=11)
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "plot_incidence_island_ga.png"), dpi=150)
plt.close()
...
ax.set_yticklabels(
    [p.split(" ", 1)[1][:22] if " " in p else p[:22] for p in prov_r2.index],
    fontsize=7)
...
ax.set_title(f"Island GA  Test R2 per Province\n(mean 2022+2023)")
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "plot_r2_heatmap_island_ga.png"), dpi=150)
plt.close()

print("  Saved plot_island_scores.png")
print("  Saved plot_incidence_island_ga.png")
print("  Saved plot_r2_heatmap_island_ga.png")
