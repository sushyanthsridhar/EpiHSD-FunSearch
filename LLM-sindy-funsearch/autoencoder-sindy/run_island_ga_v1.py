...
Uses the v1 autoencoder checkpoint: best_model_latent6.pt
val_r2 of v1 autoencoder: 0.8888

Architecture: [1612, 256, 512, 32, 6]

z5 is excluded from SINDy dynamics. The autoencoder encodes and
decodes all 6 dims as normal. During SINDy fitting and rollout,
only z1, z2, z3, z4, z6 are modelled. z5 is held fixed during
test rollout at its 2021 seed value.

Run from project root:
  python3 autoencoder-sindy/run_island_ga_v1.py

Outputs in autoencoder-sindy/island_ga_v1/
────────────────────────────────────────────────────────────────
"""
...
OUT_DIR = os.path.join(HERE, "island_ga_v1")
os.makedirs(OUT_DIR, exist_ok=True)

VERSION_LABEL = "v1"

# ═══════════════════════════════════════════════════════════════
# AUTOENCODER CONFIG for v1
# ═══════════════════════════════════════════════════════════════
LATENT_DIM      = 6
HIDDEN_DIMS     = [256, 512]
PRE_BOTTLENECK  = 32
...
MODEL_PATH = os.path.join(AE_DIR, "best_model_latent6.pt")
...
# z5 (index 4) excluded from SINDy. Held fixed during rollout.
SINDY_DIMS     = [0, 1, 2, 3, 5]      # indices into z_cols to use for SINDy
LATENT_DIM_SINDY = len(SINDY_DIMS)    # 5
...
print(f"  SINDy active dims: {SINDY_DIMS} (z5 excluded, frozen during rollout)")
...
sindy_z_cols = [z_cols[i] for i in SINDY_DIMS]   # ['z1','z2','z3','z4','z6']
...
# z_scaler fitted on all 6 dims (needed for dZ computation and rollout scale_)
...
# Polynomial library built on SINDY_DIMS only (5 dims)
poly = PolynomialFeatures(degree=POLY_DEG, include_bias=True)
z_scaled_train_sindy = z_scaler.transform(
    train_latent_ext[z_cols].values)[:, SINDY_DIMS]
poly.fit(z_scaled_train_sindy)
z_term_names = list(poly.get_feature_names_out(sindy_z_cols))
ALL_TERMS    = z_term_names + EXTERNAL_POOL_VALID
N_TERMS      = len(ALL_TERMS)
print(f"  Library terms: {N_TERMS}  (poly_deg={POLY_DEG}, "
      f"z_terms={len(z_term_names)} on {sindy_z_cols}, ext={len(EXTERNAL_POOL_VALID)})")

def build_theta(rows_df):
    z_all = z_scaler.transform(rows_df[z_cols].values.astype(float))
    z_s   = z_all[:, SINDY_DIMS]        # only the 5 SINDy dims
...
def build_pairs(df_ext, allowed_start_years, province_subset=None):
    """Returns dZ only for SINDY_DIMS, shape (n_pairs, LATENT_DIM_SINDY)."""
    Zrow_list, dZ_list = [], []
    for province, grp in df_ext.groupby("province"):
        ...
        for t in range(len(grp) - 1):
            ...
            dZ_list.append((z_t1 - z_t)[SINDY_DIMS])   # 5-dim target
    if not Zrow_list:
        return pd.DataFrame(), np.empty((0, LATENT_DIM_SINDY))
...
def eval_xi_on_val(Xi_full):
    ...
    r2s   = [r2_np(dZ_va_all[:, j], preds[:, j]) for j in range(LATENT_DIM_SINDY)]
...
# ═══════════════════════════════════════════════════════════════
print("\n" + "=" * 70)
print(f"  ISLAND SINDY TRAINING  ({VERSION_LABEL}  SINDy dims={SINDY_DIMS})")
print("=" * 70)
...
for isl in range(N_ISLANDS):
    ...
    tr_r2s   = [r2_np(dZ_tr_isl[:, j], preds_tr[:, j]) for j in range(LATENT_DIM_SINDY)]
...
def refit_xi(mask):
    idx = np.where(mask)[0]
    if len(idx) == 0:
        return np.zeros((N_TERMS, LATENT_DIM_SINDY))
    Xi_sub  = stlsq(Theta_tr_all[:, idx], dZ_tr_all, STLSQ_THRESH)
    Xi_full = np.zeros((N_TERMS, LATENT_DIM_SINDY))
    Xi_full[idx] = Xi_sub
    return Xi_full
...
coef_df = pd.DataFrame(Xi_cross, index=ALL_TERMS,
                        columns=[f"d{z}/dt" for z in sindy_z_cols])
...
for i, term in enumerate(ALL_TERMS):
    ...
    if (np.abs(row) > 1e-12).any():
        coef_str = "  ".join(f"d{z}/dt={row[j]:+.5f}"
                              for j, z in enumerate(sindy_z_cols)
                              if abs(row[j]) > 1e-12)
...
sindy_dims_arr = np.array(SINDY_DIMS)
eval_records = []
...
for province in provinces_all:
    ...
    if seed_row.empty:
        continue
    z_cur = seed_row[z_cols].values[0].astype(float)   # full 6-dim
    ...
    for test_year in TEST_YEARS:
        ...
        # Set z values to current z_cur for theta computation
        for k, zc in enumerate(z_cols):
            row_df_z[zc] = z_cur[k]
        # Clip OOD external features
        for fi, feat in enumerate(EXTERNAL_POOL_VALID):
        ...
        t_row     = build_theta(row_df_z)                    # (1, N_TERMS)
        dz_scaled = (t_row @ Xi_cross).flatten()             # (LATENT_DIM_SINDY,) = (5,)
        # Un-standardise using only the SINDY_DIMS scales
        dz_actual = dz_scaled * z_scaler.scale_[sindy_dims_arr]

        z_pred = z_cur.copy()
        z_pred[sindy_dims_arr] = z_cur[sindy_dims_arr] + dz_actual
        # z5 (index 4) stays at z_cur[4], i.e. frozen from 2021 seed
        z_cur = z_pred.copy()
...
plt.suptitle(
    f"Island SINDy GA  {VERSION_LABEL}  (dim={LATENT_DIM}  SINDy dims={SINDY_DIMS}  N_islands={N_ISLANDS})",
    fontsize=11)
plt.tight_layout()
...
print("=" * 70)
print(f"  AE val_r2 = 0.8888  (v1 checkpoint)")
print(f"  SINDy dims: {SINDY_DIMS}  (z5 excluded and frozen)")
print(f"  Incidence R2  mean={eval_df['r2_incidence'].mean():.4f}"
      f"  median={eval_df['r2_incidence'].median():.4f}")
print(f"  dI/dt     R2  mean={eval_df['r2_didt'].mean():.4f}")
