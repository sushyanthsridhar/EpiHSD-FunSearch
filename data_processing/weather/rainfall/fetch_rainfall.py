...
fetch_rainfall.py
-----------------
Fetches weekly rainfall data for each Dominican Republic province
from the Open-Meteo Historical Weather API (ERA5 reanalysis, free, no API key).

Weekly aggregation from daily precipitation_sum (mm):
  avg_rainfall  — mean daily rainfall over the 7 days of the week (mm/day)
  max_rainfall  — single wettest day in the week (mm)
  min_rainfall  — single driest day in the week (mm, often 0)

Note on interpretation:
  - avg_rainfall drives mosquito breeding site accumulation (sustained moisture)
  - max_rainfall captures extreme events that can flush breeding sites
  - min_rainfall = 0 on most dry-season weeks (expected)

Output: weekly_rainfall_by_province.csv
  columns: province, year, week, avg_rainfall, max_rainfall, min_rainfall

Merge into input.csv:
    import pandas as pd
    df      = pd.read_csv("../input.csv")
    rain    = pd.read_csv("weekly_rainfall_by_province.csv")
    df      = df.merge(rain, on=["province", "year", "week"], how="left")
...
OUTPUT_FILE = "weekly_rainfall_by_province.csv"
SLEEP_SEC   = 1.0
MAX_RETRIES = 3
RETRY_WAIT  = 5.0


def fetch_daily_rainfall(lat: float, lon: float) -> pd.DataFrame:
    """Call Open-Meteo and return a DataFrame with daily precipitation."""
    params = {
        "latitude"  : lat,
        "longitude" : lon,
    ...
    df = pd.DataFrame({
        ...
        "precip_day": data["precipitation_sum"],
    })
    # Replace any None/null values with 0 (no rain recorded = 0 mm)
    df["precip_day"] = pd.to_numeric(df["precip_day"], errors="coerce").fillna(0.0)
    return df


def daily_to_weekly(daily: pd.DataFrame) -> pd.DataFrame:
    """Aggregate daily rainfall to ISO week."""
    daily = daily.copy()
    ...
    weekly = (
        ...
        .agg(
            avg_rainfall=("precip_day", "mean"),
            max_rainfall=("precip_day", "max"),
            min_rainfall=("precip_day", "min"),
        )
        .reset_index()
    )
    weekly = weekly[weekly["week"] <= 52].copy()
    weekly["avg_rainfall"] = weekly["avg_rainfall"].round(2)
    weekly["max_rainfall"] = weekly["max_rainfall"].round(2)
    weekly["min_rainfall"] = weekly["min_rainfall"].round(2)
    return weekly
...
for i, (province, (lat, lon)) in enumerate(todo.items(), 1):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            daily  = fetch_daily_rainfall(lat, lon)
...
if not all_frames:
    ...
    print(f"\nRainfall stats (mm):")
    print(result[["avg_rainfall", "max_rainfall", "min_rainfall"]].describe().round(2))
